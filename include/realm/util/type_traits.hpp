 ...
 */
template<typename T>
inline constexpr bool is_component = (std::is_class_v<T> && std::is_default_constructible_v<T> &&
 ...
 */
template<typename... T>
inline constexpr bool is_query_args = is_unique<std::remove_const_t<pure_t<T>>...> &&
...
struct valid_query_functor<R (T::*)(Args...) const, std::enable_if_t<is_query_args<Args...>>> :
    std::true_type
{};
template<typename T, typename R, typename... Args>
struct valid_query_functor<R (T::*)(view<Args...>) const, std::enable_if_t<is_view_args<Args...>>> :
    std::true_type
{};
template<typename T>
