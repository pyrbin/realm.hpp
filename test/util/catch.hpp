...
#define INTERNAL_CATCH_NTTP_GEN(...) \
    INTERNAL_CATCH_EXPAND_VARGS( \
        INTERNAL_CATCH_VA_NARGS_IMPL(__VA_ARGS__, INTERNAL_CATCH_NTTP_1, INTERNAL_CATCH_NTTP_1, \
            ...
            for (u_int m = 0; m < count; m++) {
                ...
                if (startsWith(methodName, "Catch_TestCase_")) {
                    ...
                    getMutableRegistryHub().registerTest(makeTestCase(new OcMethod(cls, selector),
                        className, NameAndTags(name.c_str(), desc.c_str()), SourceLineInfo("", 0)));
                    noTestMethods++;
                }
            }
            free(methods);
    ...
    void run()
    {
        ...
        CATCH_TRY
        {
            ...
            BenchmarkInfo info{ name, plan.estimated_duration.count(), plan.iterations_per_sample,
                cfg->benchmarkSamples(), cfg->benchmarkResamples(),
...
IColourImpl* platformColourInstance()
{
    ...
    return colourMode == UseColour::Yes ? &s_instance : NoColourImpl::instance_();
}
} // namespace
} // end namespace Catch
...
class EnumValuesRegistry : public IMutableEnumValuesRegistry
{
    ...
    EnumInfo const& registerEnum(
        StringRef enumName, StringRef allEnums, std::vector<int> const& values) override;
};
...
public: // IMutableRegistryHub
    ...
    void registerTagAlias(
        std::string const& alias, std::string const& tag, SourceLineInfo const& lineInfo) override
    {
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker
{
    ...
    static GeneratorTracker& acquire(
        TrackerContext& ctx, TestCaseTracking::NameAndLocation const& nameAndLocation)
    {
...
void cleanupSingletons()
{
    auto& singletons_ = getSingletons();
    for (auto singleton : *singletons_)
        delete singleton;
    delete singletons_;
    singletons_ = nullptr;
}
...
void XmlReporter::benchmarkEnded(BenchmarkStats<> const& benchmarkStats)
{
    m_xml.startElement("mean")
        ...
        .writeAttribute(
            "lowerBound", static_cast<uint64_t>(benchmarkStats.mean.lower_bound.count()))
        .writeAttribute(
            "upperBound", static_cast<uint64_t>(benchmarkStats.mean.upper_bound.count()))
