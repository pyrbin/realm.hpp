...
template<typename ArgT, typename MatcherT>
auto
makeMatchExpr(ArgT const& arg, MatcherT const& matcher, StringRef const& matcherString)
...
/// in a measurement context needs to have consistent performance characteristics so that
/// we can account for it in the measurement. Implementations of std::function with
/// optimizations that aren't always applicable, like small buffer optimizations, are not
/// uncommon. This is effectively an implementation of std::function without any such
...
using TimingOf =
  Timing<ClockDuration<Clock>, Detail::CompleteType_t<Detail::ResultOf_t<Sig>>>;
} // namespace Benchmark
} // namespace Catch
...
Colour::use(Code _colourCode)
{
    ...
    if (impl != NULL) { impl->use(_colourCode); }
}
...
RegexMatcher::match(std::string const& matchee) const
{
    ...
    if (m_caseSensitivity == CaseSensitive::Choice::No) { flags |= std::regex::icase; }
                   ...
                   StringRef names)
{
    auto trimmed = [&](size_t start, size_t end) {
        while (names[start] == ',' || isspace(names[start])) { ++start; }
        while (names[end] == ',' || isspace(names[end])) { --end; }
...
{
    if (!getCurrentContext().getConfig()->showInvisibles()) { return '"' + str + '"'; }
...
{
    for (auto const& listener : m_listeners) { listener->noMatchingTestCases(spec); }
...
{
    for (auto const& listener : m_listeners) { listener->reportInvalidArguments(arg); }
...
{
    for (auto const& listener : m_listeners) { listener->benchmarkPreparing(name); }
    m_reporter->benchmarkPreparing(name);
}
void
...
{
    for (auto const& listener : m_listeners) { listener->benchmarkEnded(benchmarkStats); }
...
{
    for (auto const& listener : m_listeners) { listener->testRunStarting(testRunInfo); }
...
{
    for (auto const& listener : m_listeners) { listener->testGroupStarting(groupInfo); }
...
{
    for (auto const& listener : m_listeners) { listener->testCaseStarting(testInfo); }
    m_reporter->testCaseStarting(testInfo);
}
...
{
    for (auto const& listener : m_listeners) { listener->sectionStarting(sectionInfo); }
    m_reporter->sectionStarting(sectionInfo);
}
...
{
    for (auto const& listener : m_listeners) { listener->sectionEnded(sectionStats); }
    m_reporter->sectionEnded(sectionStats);
}
...
{
    for (auto const& listener : m_listeners) { listener->testCaseEnded(testCaseStats); }
...
{
    for (auto const& listener : m_listeners) { listener->testGroupEnded(testGroupStats); }
...
{
    for (auto const& listener : m_listeners) { listener->testRunEnded(testRunStats); }
